...
import paddle
import json
from thirdparty.utility import random_augmentation
...
DICT = {}
with open('work/tabel.json', 'r') as f:
    DICT = json.load(f)
            ...
            while True:
                for i in range(sizes):
                    ...
                    for j in range(batchsize):
                        ...
                        label = [DICT[c] for c in items[-1]]
                        # img = Image.open(os.path.join(img_root_dir, items[
                        #   2])).convert('RGB')  #zhuanhuidu
                        img = cv2.imread(os.path.join(img_root_dir, items[2]))
                        # img = Image.open(items[2]).convert('L')
                        img = random_augmentation(img)
                        if j == 0:
                            sz = img.shape
                        # print(img.size)
                        img = cv2.resize(img, (sz[1], sz[0]))
